...
function actionLoadListClassSearch(lstClassSearch){
    ...
    for(var i=0; i<lstClassSearch.length; i++){
        //up
        if(lstClassSearch[i]["class"] == "K4"){
            let sheetID = getCookie("LS2");
            let base = `https://docs.google.com/spreadsheets/d/${sheetID}/gviz/tq?`;
            checkAccAdmin = true;
            var sheetName = 'Class_Detail';
            //up
            var qu_AllData = 'Select B WHERE A = \"K4\"';
            var queryAllData = encodeURIComponent(qu_AllData);
            var urlAllData = `${base}&sheet=${sheetName}&tq=${queryAllData}`;
            fetch(urlAllData)
            .then(res => res.text())
        ...
        else if(lstClassSearch[i]["class"] == "K5"){
            let sheetID = getCookie("LS2");
            let base = `https://docs.google.com/spreadsheets/d/${sheetID}/gviz/tq?`;
            checkAccAdmin = true;
            var sheetName = 'Class_Detail';
            //up
            var qu_AllData = 'Select B WHERE A = \"K5\"';
            var queryAllData = encodeURIComponent(qu_AllData);
            var urlAllData = `${base}&sheet=${sheetName}&tq=${queryAllData}`;
            fetch(urlAllData)
            .then(res => res.text())
            ...
            var sheetName = 'Class_Detail';
            //up
            var qu_AllData = 'Select B WHERE A = \"K3\" OR A = \"K4\" OR A = \"K5\"';
            var queryAllData = encodeURIComponent(qu_AllData);
            var urlAllData = `${base}&sheet=${sheetName}&tq=${queryAllData}`;
            fetch(urlAllData)
            .then(res => res.text())
